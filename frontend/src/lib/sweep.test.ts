...
describe('one column, one proposal, every table that has it', () => {
  it('collapses the same finding across sibling tables into one group', () => {
    const groups = group([
      uptime('sessions'),
      uptime('sessions_estimated'),
      uptime('sessions_last_state'),
    ])
...
describe('a single change leaves through Infrastructure', () => {
  it('builds the link the Alter panel at the other end reads', () => {
    ...
    expect(url).toBe(
      '/infra/schema?alter=default.orders&op=modify-column&column=count&kind=UInt16',
    )
  })
...
describe('the statements come out one per table', () => {
  ...
  it('gathers every column of one table into a single ALTER', () => {
    const { sql } = statements('default', [
      member({ table: 'sessions', column: 'total_busy_seconds' }),
      member({ table: 'sessions', column: 'busy_percentage', proposal: 'UInt8' }),
      member({ table: 'sessions_estimated', column: 'total_busy_seconds' }),
    ])

    expect(sql).toHaveLength(2)
    expect(sql[0]!).toBe(
      'ALTER TABLE default.sessions\n' +
  ...
  it('says what a null becomes, only on the columns dropping their Nullable', () => {
    const { sql, cleanups } = statements('default', [
      member({ table: 'events', column: 'connected', from: 'Nullable(Bool)', proposal: 'Bool' }),
      member({ table: 'events', column: 'signal', from: 'Nullable(Int64)', proposal: 'Nullable(Int8)' }),
    ])
    ...
    expect(sql[0]!).toBe(
      'ALTER TABLE default.events\n' +
        ...
        '    MODIFY COLUMN signal    Nullable(Int8)',
    )
  })
