...
describe('reading a statement', () => {
  it('separates an equality from a range, because a key serves them in that order', () => {
    const { access } = read(
      "SELECT count(), sum(value) FROM events WHERE device_id = 'dev-1' AND time > now() - INTERVAL 7 DAY",
      'events',
      COLUMNS,
    )
  ...
  it('reads IN as an equality and LIKE as no filter at all', () => {
    ...
    const like = read("SELECT count() FROM events WHERE device_id LIKE 'dev-%'", 'events', COLUMNS)
  ...
  it('resolves a grouping through its alias and through a bucket', () => {
    const { access } = read(
      'SELECT toStartOfHour(time) AS h, count() FROM events GROUP BY h',
      'events',
      COLUMNS,
    )
  ...
  it('keeps aggregate expressions verbatim, because ClickHouse matches them that way', () => {
    ...
    const { access } = read(
      'SELECT type, count(), sum(value), quantile(0.95)(value) FROM events GROUP BY type',
      'events',
      COLUMNS,
    )
  ...
  it('refuses what it cannot read, and says which', () => {
    expect(read('INSERT INTO events VALUES', 'events', COLUMNS).refused).toBe('not-a-select')
    expect(
      read('SELECT 1 FROM events UNION ALL SELECT 2 FROM events', 'events', COLUMNS).refused,
    ).toBe('compound')
    expect(
      read('SELECT t.value FROM events AS t JOIN other AS o ON o.id = t.project_id', 'events', COLUMNS)
        .refused,
    ).toBe('joins')
    expect(read('SELECT count() FROM other_table', 'events', COLUMNS).refused).toBe('not-this-table')
    // Two columns in one conjunct is a comparison no sort order helps with.
    expect(
      read('SELECT count() FROM events WHERE project_id = value', 'events', COLUMNS).refused,
    ).toBe('opaque-filter')
    // A grouping expression the measurement endpoint would refuse anyway.
    expect(
      read(
        'SELECT toStartOfInterval(time, INTERVAL 3 HOUR) AS b, count() FROM events GROUP BY b',
        'events',
        COLUMNS,
      ).refused,
  ...
  it('does not read a filter on a function of a column as a filter on the column', () => {
    ...
    const { access } = read(
      'SELECT count() FROM events WHERE length(device_id) = 8',
      'events',
      columns,
    )
  ...
  it('follows a filter written against a select alias', () => {
    ...
    const { access } = read(
      'SELECT toStartOfHour(time) AS h, count() FROM events WHERE h > now() - INTERVAL 2 DAY GROUP BY h',
      'events',
      COLUMNS,
    )
  ...
  it('warns about a raw filter and stays quiet about a bucketed one', () => {
    ...
    const raw = candidates(
      advice([
        pattern({
          statement:
            'SELECT toStartOfHour(time) AS h, count() FROM events WHERE time > now() GROUP BY h',
        }),
      ]),
    ...
    const bucketed = candidates(
      advice([
        pattern({
          statement:
            'SELECT toStartOfHour(time) AS h, count() FROM events WHERE h > now() GROUP BY h',
        }),
      ]),
    ).find((c) => c.kind === 'aggregate')!
...
describe('what the sorting key already serves', () => {
  it('is about the first key column and nothing else', () => {
    const served = read("SELECT count() FROM events WHERE project_id = 3", 'events', COLUMNS)
    ...
    const notServed = read("SELECT count() FROM events WHERE time > now()", 'events', COLUMNS)
...
describe('candidates', () => {
  it('proposes a sort order for a filter the key cannot reach', () => {
    const list = candidates(
      advice([
        pattern({
          statement: "SELECT count(), sum(value) FROM events WHERE device_id = 'dev-1234'",
        }),
      ]),
    )
  ...
  it('proposes nothing for a filter the key already serves', () => {
    const list = candidates(
      advice([pattern({ statement: 'SELECT count() FROM events WHERE project_id = 3' })]),
    )
  ...
  it('proposes an aggregate projection for a grouping, keeping the aggregates it saw', () => {
    const list = candidates(
      advice([
        pattern({ statement: 'SELECT type, count() FROM events GROUP BY type' }),
        pattern({ statement: 'SELECT type, avg(value) FROM events GROUP BY type', hash: 'b' }),
      ]),
    )
  ...
  it('puts a filtered column into the aggregate key, since a fold cannot be filtered afterwards', () => {
    const list = candidates(
      advice([
        pattern({
          statement: "SELECT type, count() FROM events WHERE project_id = 3 GROUP BY type",
        }),
      ]),
    )
    const aggregate = list.find((c) => c.kind === 'aggregate')!
  ...
  it('ranks by the time the workload actually spends', () => {
    const list = candidates(
      advice([
        pattern({ statement: "SELECT count() FROM events WHERE device_id = 'a'", total_ms: 100, hash: 'cheap' }),
        pattern({ statement: "SELECT count() FROM events WHERE type = 'view'", total_ms: 9000, hash: 'dear' }),
      ]),
    )
  ...
  it('recognises one grouping written two ways as one proposal', () => {
    ...
    const list = candidates(
      advice([
        pattern({ statement: "SELECT type, count() FROM events WHERE project_id = 3 GROUP BY type", hash: 'a' }),
        pattern({ statement: "SELECT project_id, count() FROM events WHERE type = 'view' GROUP BY project_id", hash: 'b' }),
      ]),
  ...
  it('names the wider proposal that would serve a narrower one too', () => {
    const list = candidates(
      advice([
        pattern({ statement: 'SELECT type, count() FROM events GROUP BY type', hash: 'a' }),
        pattern({ statement: 'SELECT type, project_id, count() FROM events GROUP BY type, project_id', hash: 'b' }),
      ]),
    ).filter((c) => c.kind === 'aggregate')
  ...
  it('marks a proposal argued from one run as thin', () => {
    const list = candidates(
      advice([
        pattern({ statement: "SELECT count() FROM events WHERE device_id = 'a'", runs: 1, total_ms: 3, hash: 'once' }),
        pattern({ statement: "SELECT count() FROM events WHERE type = 'x'", runs: 200, total_ms: 9000, hash: 'often' }),
      ]),
    )
  ...
  it('says when a proposal has folded in more aggregates than it should', () => {
    const shapes = ['count()', 'sum(value)', 'avg(value)', 'min(value)', 'max(value)'].map((agg, i) =>
      pattern({ statement: `SELECT type, ${agg} FROM events GROUP BY type`, hash: `agg${i}` }),
    )
  ...
  it('carries the patterns it could not read, rather than dropping them', () => {
    const report = advice([
      pattern({ statement: 'SELECT t.value FROM events AS t JOIN devices AS d ON d.id = t.device_id' }),
    ])
...
describe('the statements', () => {
  it('writes an aggregate projection as a grouped select with no FROM', () => {
    const candidate = candidates(
      advice([pattern({ statement: 'SELECT type, count(), sum(value) FROM events GROUP BY type' })]),
    )[0]!
    const ddl = ddlFor(candidate, 'lab', 'events', [])
    expect(ddl.query).toBe('SELECT type, count(), sum(value) GROUP BY type')
    expect(ddl.declare).toContain('ALTER TABLE lab.events')
  ...
  it('writes a sort projection over only the columns the workload reads', () => {
    const candidate = candidates(
      advice([
        pattern({ statement: "SELECT device_id, value FROM events WHERE device_id = 'a'" }),
      ]),
    )[0]!
    const ddl = ddlFor(candidate, 'lab', 'events', [])
  ...
  it('quotes a column name that needs it', () => {
    ...
    const candidate = candidates(
      advice([pattern({ statement: 'SELECT value FROM events WHERE `odd name` = 1' })], {
        columns,
      }),
    )[0]!
    expect(ddlFor(candidate, 'lab', 'events', []).query).toBe(
...
describe('the arithmetic', () => {
  ...
  it('counts an aggregate projection rather than modelling it', () => {
    ...
    const candidate = candidates(
      advice([
        pattern({
          statement: 'SELECT type, count() FROM events GROUP BY type',
          runs: 3,
...
describe('what is said about the cost', () => {
  it('gives a sort projection the bytes its columns hold today, and the share', () => {
    const candidate = candidates(
      advice([pattern({ statement: "SELECT value FROM events WHERE device_id = 'a'" })]),
    )[0]!
  ...
  it('gives an aggregate projection a row count and no bytes, because bytes would be a guess', () => {
    const candidate = candidates(
      advice([pattern({ statement: 'SELECT type, count() FROM events GROUP BY type' })]),
    ).find((c) => c.kind === 'aggregate')!
...
describe('taking an aggregate apart for the weigher', () => {
  ...
  it('refuses the whole request when one aggregate cannot be taken apart', () => {
    ...
    const mixed = candidates(
      advice([
        pattern({ statement: 'SELECT type, count() FROM events GROUP BY type', hash: 'a' }),
        pattern({ statement: 'SELECT type, sum(value * 2) FROM events GROUP BY type', hash: 'b' }),
      ]),
    ).find((c) => c.kind === 'aggregate')!
...
describe('projections that are already there and not earning', () => {
  ...
  it('names one that is built and unused, with the reason to think first', () => {
    const [found] = standing(
      advice([pattern({ statement: 'SELECT count() FROM events' })], {
...
describe('what the cap left out', () => {
  it('says nothing when the list is the whole workload', () => {
    const counts = tally(advice([pattern({ statement: 'SELECT count() FROM events', runs: 4 })]))
  ...
  it('and reports both totals when it is a truncation', () => {
    ...
    const counts = tally(
      advice([pattern({ statement: 'SELECT count() FROM events', runs: 4 })], {
        shapes_total: 213,
        runs_total: 4100,
      }),
    )
  ...
  it('names a table whose costliest shape no key serves', () => {
    const [r] = ranked(
      report([
        table({
          table: 'events',
          samples: [pattern({ statement: "SELECT count() FROM events WHERE device_id = 'a'" })],
        }),
      ]),
    )
  ...
  it('and one whose costliest shape the key already serves', () => {
    const [r] = ranked(
      report([
        table({
          table: 'events',
          samples: [pattern({ statement: 'SELECT count() FROM events WHERE project_id = 3' })],
        }),
      ]),
    )
      ...
      report([
        table({
          ...
          samples: [
            pattern({
              statement: 'SELECT type, count() FROM events GROUP BY type',
              projections: ['lab.events.p_by_type'],
            }),
          ],
        }),
      ]),
      ...
      report([
        table({
          ...
          samples: [
            ...
            pattern({
              statement: 'SELECT count() FROM events AS t JOIN devices AS d ON d.id = t.device_id',
              tables: ['lab.events', 'lab.devices'],
              hash: 'join',
            }),
            pattern({
              statement: 'SELECT type, count() FROM events GROUP BY type',
...
describe('the readings that are not findings', () => {
  ...
  it('does not call a shape with no filter and no grouping a candidate', () => {
    ...
    const [r] = ranked(
      report(table({ table: 'events', samples: [pattern({ statement: 'SELECT count() FROM events' })] })),
    )
  ...
  it('nor one that reads columns but filters on nothing', () => {
    const [r] = ranked(
      report(
        table({
          table: 'events',
          samples: [pattern({ statement: 'SELECT device_id, value FROM events LIMIT 100' })],
        }),
      ),
    )
    expect(r!.verdict).toBe('unserveable')
      ...
      tables: [
        table({
          ...
          samples: [
            ...
            pattern({ statement: 'SELECT uniqCombined(value) FROM events', hash: 'profile' }),
            pattern({ statement: "SELECT count() FROM events WHERE device_id = 'a'", hash: 'real' }),
          ],
        }),
      ],
    })
